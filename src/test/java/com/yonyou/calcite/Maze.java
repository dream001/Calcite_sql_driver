...
public class Maze {
     ...
     *        Print writer
     * @param space
     *        Whether to put a space in each cell; if false, prints the
    ...
    private void row(CellContent cellContent, StringBuilder b, StringBuilder b2, int y) {
        ...
        for (int x = 0; x < width; x++) {
            b2.append(lefts[c0 + x] ? ' ' : '|').append(cellContent.get(c0 + x));
        }
        b2.append('|');
    }

    public Maze layout(Random random, PrintWriter pw) {
        int[] candidates = new int[width * height - width + width * height - height];
        int z = 0;
